...
const ModelAction = ({
    ...
    mutationConfig: {
      onSuccess: (data) => {
        ...
        const conflatedResults = handleConflation(
          ...
          {
            ...getTrainingConfig(),
            zoom_level: predictionZoomLevel ?? currentZoom, // Ensure zoom level used during prediction is retained
          },
        );
  ...
  const handlePrediction = useCallback(async () => {
    if (!map) return;
    setPredictionZoomLevel(currentZoom); // Set the zoom level used during prediction
